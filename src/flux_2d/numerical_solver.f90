...
contains
    ...
    subroutine rk4(ax, ay, vx, vy, diffusivity, boundary_condition, dx, dy, dt, nsteps, nx, ny, start, number_of_saves, identifier)
        ...
        integer, intent(in) :: boundary_condition, nx, ny, nsteps, number_of_saves
        real, intent(in) :: vx(:,:), vy(:,:), diffusivity, dx, dy, dt, start
        ...
        do i=1, nsteps
        call f(ax, ay, vx, vy, diffusivity, boundary_condition, dx, dy, nx, ny, x1, y1)
        call f(ax + 0.5*dt*x1, ay + 0.5*dt*y1, vx, vy, diffusivity, boundary_condition, dx, dy, nx, ny, x2, y2)
        call f(ax + 0.5*dt*x2, ay + 0.5*dt*y2, vx, vy, diffusivity, boundary_condition, dx, dy, nx, ny, x3, y3)
        call f(ax + dt*x3, ay + dt*y3, vx, vy, diffusivity, boundary_condition, dx, dy, nx, ny, x4, y4)
        ax = ax + (dt/6)*(x1 + 2*x2 + 2*x3 + x4)
        ay = ay + (dt/6)*(y1 + 2*y2 + 2*y3 + y4)

        call solver_progress(i, nsteps, start, number_of_saves, identifier, ax, ay, dx, dy, nx, ny, dt)
        end do
    end subroutine rk4


    subroutine euler(ax, ay, vx, vy, diffusivity, boundary_condition, dx, dy, dt, nsteps, nx, ny, start, number_of_saves, identifier)
        real, intent(in) :: vx(:,:), vy(:,:), diffusivity, dx, dy, dt, start
        real, intent(inout) :: ax(:,:), ay(:,:)
        integer, intent(in) :: boundary_condition, nx, ny, nsteps, number_of_saves
        character(512), intent(in) :: identifier
        ...
        do i=1, nsteps
        call f(ax, ay, vx, vy, diffusivity, boundary_condition, dx, dy, nx, ny, kx, ky)
        ax = ax + dt*kx
        ay = ay + dt*ky
    ...
    subroutine heun(ax, ay, vx, vy, diffusivity, boundary_condition, dx, dy, dt, nsteps, nx, ny, start, number_of_saves, identifier)
        real, intent(in) :: vx(:,:), vy(:,:), diffusivity, dx, dy, dt, start
        real, intent(inout) :: ax(:,:), ay(:,:)
        integer, intent(in) :: boundary_condition, nx, ny, nsteps, number_of_saves
        character(512), intent(in) :: identifier
        ...
        do i=1, nsteps
        call f(ax, ay, vx, vy, diffusivity, boundary_condition, dx, dy, nx, ny, k1x, k1y)
        call f(ax + dt*k1x, ay + dt*k1y, vx, vy, diffusivity, boundary_condition, dx, dy, nx, ny, k2x, k2y)
    ...
    subroutine solver_progress(i, nsteps, start, number_of_saves, identifier, ax, ay, dx, dy, nx, ny, dt)
        integer, intent(in) :: i, nsteps, number_of_saves, nx, ny
        ...
        save_points = [ ( floor(real(nsteps)/(real(number_of_saves)-1.0))*k,k=1,number_of_saves-1 ) ]

        if (number_of_saves .gt. 2) then
            if ((i == nsteps) .or. (any(save_points==i))) then
                ! save system
                call save_snapshot(.false., identifier, ax, ay, dx, dy, nx, ny)
                ! save corresponding timestamp
                call save_timestamp(identifier, (start+i*dt))
            end if
        end if
    end subroutine solver_progress


    subroutine save_snapshot(is_initial, identifier, ax, ay, dx, dy, nx, ny, vx, vy)
        real, intent(in) :: ax(:,:), ay(:,:), dx, dy
        ...
        if (is_initial) then
            mode = "asis"
            call export_data(identifier, vx, "vx.dat", trim(mode))
            call export_data(identifier, vy, "vy.dat", trim(mode))
        else
            mode = "append"
        end if
        ...
        call calculate_j(dx, dy, nx, ny)
        call export_data(identifier, b(2:nx+1,2:ny+1), "bz.dat", trim(mode)) ! exclude extrapolated points
        call export_data(identifier, ax, "ax.dat", trim(mode))
        call export_data(identifier, ay, "ay.dat", trim(mode))
        if (present(vx)) call export_data(identifier, vx, "vx.dat", trim(mode))
        if (present(vy)) call export_data(identifier, vy, "vy.dat", trim(mode))
        call export_data(identifier, jx, "jx.dat", trim(mode))
        call export_data(identifier, jy, "jy.dat", trim(mode))
